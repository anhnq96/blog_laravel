            ...
            <div class="row">
                <div class="col-xl-6 col-lg-12">
                    ...
                    <div class="box">
                        ...
                        <!-- /.box-header -->
                        <div class="box-body">
                            <form novalidate method="post" action="{{ route('admin.tag.update', $tag->id)  }}">
                                @csrf
                                {{ method_field('PUT') }}
                                <div class="form-group">
                                    <h5>Title <span class="text-danger">*</span></h5>
                                    <div class="controls">
                                        <input type="text" value="{{ $tag->title }}" name="title" class="form-control" required data-validation-required-message="This field is required">
                                    </div>
                                </div>
                                <div class="form-group">
                                    <h5>Description</h5>
                                    <div class="controls">
                                        <input type="text" value="{{ $tag->description }}" name="description" class="form-control">
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label>Status</label>
                                    <select class="form-control" name="status">
                                        <option value="0" {{ $tag->status == 0  ? 'selected' : ''}}>Active</option>
                                        <option value="10" {{ $tag->status == 10  ? 'selected' : ''}}>Inactive</option>
                                    </select>
                                </div>
                                <div class="form-group">
